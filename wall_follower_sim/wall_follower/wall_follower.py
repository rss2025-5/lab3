...
class WallFollower(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("%d" % self.SIDE)
    ...
    def scan_callback(self, msg):
        ...
        tracked_wall = side_wall_filter(self.SIDE)
        ...
        P = 3 * error
        hz = 1/msg.time_increment if msg.time_increment > 0 else 1
        D = 0 * (error - self.prev_error) * hz
        I = 1 * self.integral_error
        ...
        if len(forward_wall[0]) < 0.8 * f_wall_pts:
            m = 0
            b = 0
            dist = np.inf
        else:
        ...
        km = 20
        if dist <= e_stop_dist and abs(m) < 0.5:
            steering_angle = np.clip(
        ...
        send_vel = self.VELOCITY*(1-steering_angle/180); 

        self.drive(send_vel, steering_angle)
