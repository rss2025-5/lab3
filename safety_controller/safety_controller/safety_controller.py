...
class SafetyController(Node):
    ...
    def __init__(self):
        ...
        self.INTERCEPT_TOPIC = "/vesc/low_level/ackermann_cmd"
    ...
    def scan_callback(self, scan):
        ...
        forward_wall = forward_wall[:, forward_wall[0,:] <= 3*self.STOP_DISTANCE]
        if len(forward_wall[0]) < 0.7 * f_wall_pts or len(forward_wall[0]) == 0:
            m = 0
            b = 0
            dist = np.inf
        else:
        ...
        if dist <= 1.5*self.STOP_DISTANCE or min_dist <= 0.5*self.STOP_DISTANCE:
            self.drive(0.0, 0.0)
